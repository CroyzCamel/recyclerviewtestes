...
import com.example.recycler_view_testes.databinding.ActivityMain2Binding
...
class MainActivity2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.btAcitivty3.setOnClickListener {
            ...
            val call = apiServices.getPostsId(id).enqueue(object: Callback<List<PostsComents>>{
                override fun onResponse(call: Call<List<PostsComents>>, response: Response<List<PostsComents>>) {
                    if (response.isSuccessful) {
                        val body = response.body()
                        if (body != null){
                            val adapter = PostsComentsAdapter(this@MainActivity2,body)
        ...
        binding .btActivity3.setOnClickListener {
